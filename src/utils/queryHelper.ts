...
export const getSearchMaterials = ({
    query,
    options,
    queryModel,
}: {
    ...
    let queryCopy: any = structuredClone(query);
...
export async function listQueryHelper(
    queryModel: any,
    queries?: any,
...
export const singleQueryHelper = async (
    queryModel: any,
    queries?: any,
    options?: Record<string, any>
) => {
    const { searchQuery, projection, sortQuery, populate } = getSearchMaterials(
        {
            query: queries,
            options,
            queryModel,
    ...
    return await queryModel
        .findOne({ ...searchQuery }, projection, {
            populate,
            ...options,
        })
        .sort(sortQuery);
